...
import 'package:instaclone/results/eyes/eyelidshaperesults.dart';
import 'package:instaclone/scancards/creditspage.dart';

class Eyelidshapemesh extends StatefulWidget {
  final File imageFile; // Image file passed from ScanEyeQualityCards

  const Eyelidshapemesh({super.key, required this.imageFile});

  @override
  EyelidshapemeshState createState() => EyelidshapemeshState();
}

class EyelidshapemeshState extends State<Eyelidshapemesh> {
...
// Load ui.Image
  ...
  void _analyzeEyeShape() {
    ...
    if (leftEyeIndices.every((index) => index < _faceMeshPoints!.length) &&
      ...
      analyzeEyeShape(leftEyeWidth, leftEyeHeight);
      analyzeEyeShape(rightEyeWidth, rightEyeHeight);

      String leftEyelidShape =
          analyzeEyelidShape(leftEyeUpperLidHeight, leftEyeHeight);
      String rightEyelidShape =
          analyzeEyelidShape(rightEyeUpperLidHeight, rightEyeHeight);

     // print('Left Eye Shape: $leftEyeShape');
     // print('Right Eye Shape: $rightEyeShape');
 ...
 WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        context,
        MaterialPageRoute(
          builder: (context) => Eyelidshaperesults(
            image: widget.imageFile,
            results: 'Left Eyelid Shape: $leftEyelidShape, Right Eyelid Shape: $rightEyelidShape',
           ),
          ),
        );
      });
