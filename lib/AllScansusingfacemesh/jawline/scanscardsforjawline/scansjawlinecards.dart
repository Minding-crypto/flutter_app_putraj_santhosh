...
import 'package:instaclone/AllScansusingfacemesh/jawline/jawlinemesh.dart';
...
class Scansjawlinemeshcards extends StatefulWidget {
  const Scansjawlinemeshcards({super.key});

  @override
  ScansjawlinemeshcardsState createState() => ScansjawlinemeshcardsState();
}

class ScansjawlinemeshcardsState extends State<Scansjawlinemeshcards> {
      ...
      'text': const Row(
        children: [
          Text(
            '✔ ',
            style:
                TextStyle(color: Color.fromARGB(255, 0, 255, 26), fontSize: 18),
          ),
          SizedBox(
              width: 5), // Adjust the spacing between the icon and the text
          Flexible(
            child: Text(
              'Full Front Profile Directly at Eye Level',
...
// Use the cropped image without resizing
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => Jawlinemesh(
                  imageFile: File(croppedImage
                      .path)), // Assuming FaceMeshScreen takes an imageFile argument
            ),
          );
        }
      }
