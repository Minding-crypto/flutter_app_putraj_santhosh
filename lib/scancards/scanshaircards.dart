...
import 'package:instaclone/results/hairresults.dart';
...
class ScanshairCards extends StatefulWidget {
  const ScanshairCards({super.key});

  @override
  ScanshairCardsState createState() => ScanshairCardsState();
}

class ScanshairCardsState extends State<ScanshairCards> {
...
Future<void> _deductCredits(int creditsToDeduct) async {
  ...
  final List<Map<String, dynamic>> _secondPageInstructions = [
     {
      'image': 'assets/20231223065305W01y8M_jpg.rf.b969f4fac397f3c9c81611adc3dfda19.jpg',
      'text': const Row(
  children: [
    Text( '✔ ',
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (croppedImage != null) {
          ...
          int targetWidth = 640;
          int targetHeight = (640 / aspectRatio).round();
  ...
  loadModel() async {
    await Tflite.loadModel(
      model: "assets/modelforhairgrading.tflite",
      labels: "assets/labelsforhairgrading.txt",
    );
  }
    ...
    try {
      ...
      if (_recognitions != null && _recognitions.isNotEmpty) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
               builder: (context) => Hairresults(results: v, image: _image!),
          ),
        );
      } else {
